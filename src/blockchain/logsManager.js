...
export default class LogsManager {
  ...
  checkBlock (blockNumber, block, caver) {
    async.eachOf(block.transactions, (tx, i, next) => {
      let txHash = '0x' + tx.hash().toString('hex')

      caver.klay.getTransactionReceipt(txHash, (_error, receipt) => {
        for (let log of receipt.logs) {
          ...
          for (let subscriptionId of subscriptions) {
            ...
            let response = { 'jsonrpc': '2.0', 'method': 'klay_subscription', params: { 'result': result, 'subscription': subscriptionId } }
