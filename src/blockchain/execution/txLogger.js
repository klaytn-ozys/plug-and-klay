...
class TxLogger {
	constructor(blockchain) {
		...
		this.logKnownTXAsJSON = (args, cmds, append) => {
			var data = args;
			var el;
			if (data.tx.isCall) {
				el = renderCallAsJSON(this, data);
			} else {
				el = renderKnownTransactionAsJSON(this, data, blockchain);
			}
		...
		this.logUnknownTXAsJSON = (args, cmds, append) => {
			// liiggered for transaction AND call
			var data = args;
			var el = renderUnknownTransactionAsJSON(this, data, blockchain);
...
function renderKnownTransactionAsJSON(self, data, blockchain) {
	var from = data.tx.from;
	var to = data.resolvedData.contractName + '.' + data.resolvedData.fn;
	var obj = { from, to };
	var txType = 'knownTx';
	...
	return {
		...checkTxStatusAsJSON(data.receipt, txType),
		...contextAsJSON(self, { from, to, data }, blockchain),
		...txDetailsAsJSON(data, obj)
	}
}
...
function renderCallAsJSON(self, data) {
	var to = data.resolvedData.contractName + '.' + data.resolvedData.fn;
	var from = data.tx.from ? data.tx.from : ' - ';
	var obj = { from, to };
	var txType = 'call';
...
function renderUnknownTransactionAsJSON(self, data, blockchain) {
	var from = data.tx.from;
	var to = data.tx.to;
	var obj = { from, to };
	var txType = 'unknown' + (data.tx.isCall ? 'Call' : 'Tx');
	let tx = data.receipt || data.tx

	return {
		...checkTxStatusAsJSON(tx, txType),
		...contextAsJSON(self, { from, to, data }, blockchain),
		...txDetailsAsJSON(data, obj)
	};
}
...
function checkTxStatusAsJSON(tx, type) {
	let status;

	if (tx.status === '0x1' || tx.status === true) {
		status = 'succeed'
	} else if (type === 'call' || type === 'unknownCall') {
		status = 'call'
	} else if (tx.status === '0x0' || tx.status === false) {
		status = 'failed'
	} else {
...
function contextAsJSON(self, opts, blockchain) {
	var data = opts.data || '';
	var from = opts.from ? helper.shortenHexData(opts.from) : '';
	var to = opts.to;
	if (data.tx.to) to = to + ' ' + helper.shortenHexData(data.tx.to);
...
function txDetailsAsJSON(data, obj) {
	const from = obj.from;
	const to = obj.to;

	if (data.tx && data.tx.returnValue) {
		delete data.tx.returnValue
	}
	...
	return {
		...
		gas: data.tx.gas,
		input: data.tx.input,
		'decoded input': data.resolvedData && data.resolvedData.params ? typeConversion.stringify(data.resolvedData.params) : ' - ',
		'decoded output': data.resolvedData && data.resolvedData.decodedReturnValue ? typeConversion.stringify(data.resolvedData.decodedReturnValue) : ' - ',
		logs: data.logs,
		val: data.tx.value,
		transactionCost: data.tx.transactionCost,
		executionCost: data.tx.executionCost,
